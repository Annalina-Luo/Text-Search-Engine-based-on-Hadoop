...
public class TFIDF_InvertedIndex {
    ...
    public static class TFIDFReducer extends Reducer<Text, Text, Text, Text> {
        protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            for (Text value : values) {
                fileList1.add(value.toString());
                count += 1;
            }

            //todo, sort fileList, the inverted index for ranking purpose
	    for(int i = 0; i < count; i++){
	    	String str = fileList1.get(i);
	    	String c_str = str.substring(0,str.indexOf(":"));
	    	int num = Integer.parseInt(c_str);
	    	counts.add(num);
	    }         
